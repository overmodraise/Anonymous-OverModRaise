...
void eval_poly_deg31(const double u[32],
    ...
    // bottom level
    R_Q<LOGQ, N>* L0 = new R_Q<LOGQ, N>[32];  
    for (int n = 0; n < 32; n++) {
        L0[n].setzero();
    ...
    // level one
    const R_Q_square<LOGQ, N>& T1 = ct;
    R_Q_square<LOGQ - LOGDELTA, N>* L1 = new R_Q_square<LOGQ - LOGDELTA, N>[16]; 
    for (int n = 0; n < 16; n++) {
    ...
    R_Q_square<LOGQ - LOGDELTA * 2, N>* L2 = new R_Q_square<LOGQ - LOGDELTA * 2, N>[8];  

    for (int n = 0; n < 8; n++) {
        R_Q_square<LOGQ - LOGDELTA, N> temp;
        Mul<LOGQ - LOGDELTA, N>(T2, L1[2 * n + 1], evk2, temp);
        L1[2 * n] *= 1ULL << LOGDELTA;
        temp += L1[2 * n];
        RS<LOGQ - LOGDELTA, LOGQ - LOGDELTA * 2, N>(temp, L2[n]);
    }
    ...
    {
        R_Q_square<LOGQ - LOGDELTA, N> temp;
        Mul<LOGQ - LOGDELTA, N>(T2, T2, evk2, temp);
        temp *= 2;
        Z_Q<LOGQ - LOGDELTA> one;
        one.setzero();
        one[0] = 1ULL << LOGDELTA;
        one *= 1ULL << LOGDELTA;
        temp[0][0] -= one;
    ...
    R_Q_square<LOGQ - LOGDELTA * 3, N>* L3 = new R_Q_square<LOGQ - LOGDELTA * 3, N>[4];  

    for (int n = 0; n < 4; n++) {
        R_Q_square<LOGQ - LOGDELTA * 2, N> temp;
        Mul<LOGQ - LOGDELTA * 2, N>(T4, L2[2 * n + 1], evk3, temp);
        L2[2 * n] *= 1ULL << LOGDELTA;
        temp += L2[2 * n];
        RS<LOGQ - LOGDELTA * 2, LOGQ - LOGDELTA * 3, N>(temp, L3[n]);
    }
    ...
    {
        R_Q_square<LOGQ - LOGDELTA * 2, N> temp;
        Mul<LOGQ - LOGDELTA * 2, N>(T4, T4, evk3, temp);
        temp *= 2;
        Z_Q<LOGQ - LOGDELTA * 2> one;
        one.setzero();
        one[0] = 1ULL << LOGDELTA;
        one *= 1ULL << LOGDELTA;
        temp[0][0] -= one;
    ...
    R_Q_square<LOGQ - LOGDELTA * 4, N>* L4 = new R_Q_square<LOGQ - LOGDELTA * 4, N>[2];  

    for (int n = 0; n < 2; n++) {
        R_Q_square<LOGQ - LOGDELTA * 3, N> temp;
        Mul<LOGQ - LOGDELTA * 3, N>(T8, L3[2 * n + 1], evk4, temp);
        L3[2 * n] *= 1ULL << LOGDELTA;
        temp += L3[2 * n];
    ...
    {
        R_Q_square<LOGQ - LOGDELTA * 3, N> temp;
        Mul<LOGQ - LOGDELTA * 3, N>(T8, T8, evk4, temp);
        temp *= 2;
        Z_Q<LOGQ - LOGDELTA * 3> one;
        one.setzero();
        one[0] = 1ULL << LOGDELTA;
        one *= 1ULL << LOGDELTA;
        temp[0][0] -= one;
