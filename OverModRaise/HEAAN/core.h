...
template< int LOGQfr, int LOGQto, int N>
void Inv_RS(const R_Q_square<LOGQfr, N>& ct_fr,
	R_Q_square<LOGQto, N>& ct_to) {
	for (int i = 0; i < N; i++) {
		shift_left<LOGQfr, LOGQto>(ct_fr[0][i], ct_to[0][i]);
		shift_left<LOGQfr, LOGQto>(ct_fr[1][i], ct_to[1][i]);
	}
}
...
// Assuming the necessary HEAAN headers are included
// and the R_Q, R_Q_square, conv, resize, RS functions/classes are defined appropriately.
...
void Mul(const R_Q_square<LOGQ, N>& ct1,
    ...
    // Allocate d2 on the heap
    auto d2 = std::make_unique<R_Q<LOGQ, N>>();
    conv<LOGQ, N>(ct1[1], ct2[1], *d2);
    
    // Allocate d2_ on the heap
    auto d2_ = std::make_unique<R_Q<2 * LOGQ, N>>();
    resize<LOGQ, 2 * LOGQ, N>(*d2, *d2_);
    
    // Allocate evk_d2 on the heap
    auto evk_d2 = std::make_unique<R_Q_square<2 * LOGQ, N>>(evk);
    *evk_d2 *= *d2_;
    
    // Perform the RS operation
    RS<2 * LOGQ, LOGQ, N>(*evk_d2, ct3);
    
    // Allocate d0d1 on the heap
    auto d0d1 = std::make_unique<R_Q_square<LOGQ, N>>();
    ...
    // Smart pointers automatically deallocate memory
}